...
class Upgrader {
	 ...
	 *  'freemius' => '\WBCR\Factory_483\Updates\Wordpress_Repository'
	 * ]
	 */
	...
	public function __construct(Wbcr_Factory483_Plugin $plugin)
	{
		...
		self::$repositories['wordpress'] = '\WBCR\Factory_483\Updates\Wordpress_Repository';
		self::$repositories['github'] = '\WBCR\Factory_483\Updates\Github_Repository';
	...
	protected function get_repository($repository_name)
	{
		...
		if( isset(self::$repositories[$repository_name]) && class_exists(self::$repositories[$repository_name]) ) {
			if( self::$repositories[$repository_name] instanceof Repository ) {
				throw new Exception("Repository {$repository_name} must extend the class WBCR\Factory_483\Updates\Repository interface!");
			}
